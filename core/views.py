...
from .admin import TypePersonResource, PersonResource, ClassificationNewsResource, TypeNewsResource, VehicleResource, \
    MaterialResource, NewsResource
from .serializers import TypePersonDefaultSerializer, PersonDefaultSerializer, ClassificationNewsDefaultSerializer, \
    TypeNewsDefaultSerializer, VehicleDefaultSerializer, MaterialDefaultSerializer, NewsDefaultSerializer
...
class ClassificationNewsViewSet(ModelViewSet):
    queryset = ClassificationNews.objects.all()
    serializer_class = ClassificationNewsDefaultSerializer
    filter_backends = [DjangoFilterBackend, SearchFilter]
    search_fields = ['description']
    ...
    @action(methods=['GET'], detail=False)
    def export(self, request):
        dataset = ClassificationNewsResource().export()
        return Response(dataset.csv, status=status.HTTP_200_OK)

    @action(methods=['POST'], detail=False)
    def _import(self, request):
        try:
            resource = ClassificationNewsResource()
            errors = []
            invalids = []
            if request.FILES:
                file = request.FILES['file']
    ...
    def field_options(self, request):
        ...
        if field:
            try:
                choices = []
                for c in ClassificationNews._meta.get_field(field).choices:
                    choices.append({
                        "value": c[0],
                        "description": c[1]
                    })
